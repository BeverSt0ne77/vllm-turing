...
class FireflyAllReduce:
    ...
    def __init__(
        self, rank_in_group: int, world_size: int, device, shm_name: str, group=None
    ):
        ...
        if world_size >= 4:
            ...
            self._work = torch.empty(
                self._data_half, dtype=torch.float16, device=self.device
            )
        self.disabled = False
        logger.info(
    ...
    def _init_p2p(self, total: int) -> bool:
        ...
        try:
            ...
            self._ptrs = CustomAllreduce.create_shared_buffer(total, group=self._group)
            ...
            for r in range(2):
                if r != self.rank:
                    self._mod.firefly_ar_zero_meta(self._peer_base, self._data_half, 2)
            torch.cuda.synchronize()
            return True
        except Exception as e:  # noqa: BLE001 - IPC 不可用回 SHM
            logger.warning("firefly ar P2P init failed, fallback SHM: %s", e)
            return False
    ...
    def _init_p2p_butterfly(self) -> bool:
        ...
        try:
            ...
            for r in range(n):
                self._mod.firefly_ar_zero_meta(self._ptrs[r], self._data_half, n)
            torch.cuda.synchronize()
            return True
        except Exception as e:  # noqa: BLE001 - IPC/P2P 不可用回退 NCCL
            logger.warning("firefly ar butterfly init failed, fallback NCCL: %s", e)
            self._bases_dev = None
            return False
    ...
    def _init_shm_n(self, total: int, shm_name: str) -> bool:
        ...
        self._xq = torch.empty(self._data_half, dtype=torch.uint8, device=self.device)
        self._xq_peer = torch.empty(
            self._data_half, dtype=torch.uint8, device=self.device
        )
    ...
    def should_firefly_ar(self, inp: torch.Tensor) -> bool:
        ...
        return nbytes >= envs.VLLM_FIREFLY_AR_MIN_SIZE
