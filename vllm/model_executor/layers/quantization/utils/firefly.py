...
def _load_cuda_mod():
    ...
    try:
        cu_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "firefly.cu")
        if not os.path.exists(cu_path):
            logger.warning(
                "firefly kernel source (firefly.cu) not found at %s; use PyTorch path",
                cu_path,
            )
            return None
        ...
        logger.warning("firefly CUDA ext load failed, fallback to PyTorch: %s", e)
        _cuda_mod = None
    return _cuda_mod
...
def _unpack_awq_zp(
    ...
    """AWQ 干净 qzeros [N/8, K/gs] int32 -> 干净 zp [N, K/gs] float。

    打包时 nibble j = N offset j (见下)。镜像
    auto_awq._convert_awq_to_standard_format 的 qzeros 打包:
    new_qz[n//8, g] = Σ_j zp[n//8*8+j, g] << 4j。
    """
...
def _reverse_repack_indices(
    size_k: int, size_n: int, device
...
def dequant_marlin_to_int8(
    ...
    w_int8 = torch.clamp(torch.round(w_deq / c_n.unsqueeze(1)), -127, 127).to(
        torch.int8
    )
    return w_int8, c_n
...
def dequant_marlin_to_int8_cached(
    ...
    if use_recip:
        ...
        return torch.clamp(torch.round(w_deq * r.unsqueeze(1)), -127, 127).to(
            torch.int8
        )
    return torch.clamp(torch.round(w_deq / c_n.unsqueeze(1)), -127, 127).to(torch.int8)
